const processAPIData = (rawData) => {
    ...
    for (const [key, obj] of Object.entries(rawData)) {
        ...
        for (const [date, val] of Object.entries(obj)) {
            ...
            if (index > 0) {
                processedData[newKey].push(processedData[key][index] - processedData[key][index - 1]);

                if (newKey === "newCases") {
                    processedData.newActive.push(processedData[newKey][index - 1]);
                } else {
                    processedData.newActive[index - 1] = processedData.newActive[index - 1] - processedData[newKey][index - 1];
                }
            }
