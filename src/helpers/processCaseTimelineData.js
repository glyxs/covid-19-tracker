const processCaseTimelineData = (data, sort) => {
    ...
    const chartData = {};
    const colors = { cases: 'rgba(128, 128, 128, 0.5)', active: 'rgba(11, 196, 245, 0.5)', recovered: 'rgba(45, 210, 73, 0.5)', deaths: 'rgba(255, 14, 0, 0.5)' };
    const borderColors = { cases: 'rgba(128, 128, 128, 1)', active: 'rgba(11, 196, 245, 1)', recovered: 'rgba(45, 210, 73, 1)', deaths: 'rgba(255, 14, 0, 1)' };
    ...
    for (const [key, arr] of Object.entries(data)) {
        if (!key.includes('new') && key !== 'dates') {
            ...
            if (chartData[key][chartData[key].length - 1] !== arr[i]) {
                ...
                chartData[key].datasets[0].backgroundColor.push('rgba(255, 159, 16, 0.5)');
            }
        }
    }
    return chartData;
